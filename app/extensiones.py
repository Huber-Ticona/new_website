...
def verificar_pdf(cotizacion_id):
    x = 0
    ruta = current_app.config['COTIZACION_FOLDER'] + \
        "\cotizacion_" + str(cotizacion_id) + ".pdf"
...
def generar_pdf(datos):
    ...
    # A4 (210 * 297 mm)
    url = current_app.config['COTIZACION_FOLDER'] + \
        "\cotizacion_" + str(datos['cotizacion_id']) + ".pdf"
    ...
    tabla_productos = [["Codigo", "Descripcion",
                        "U/M", "Cantidad", "Precio", "Total"]]

    monto_total = 0
    for item in datos['lista_productos']:
        tabla_productos.append(
            ['xxxx', item[3], 'Plancha', item[2], item[4], (item[2] * item[4])])
        monto_total = item[5]
